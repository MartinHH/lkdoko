 ...
 */
case class AckConfig(autoAckTimeout: Int, ack: Acknowledgement[GameState])
...
object AckConfig:
  ...
  def forState(state: GameState): Option[AckConfig] = {
    def short(ack: Acknowledgement[GameState]) = Some(AckConfig(shortTimeout, ack))
    def long(ack: Acknowledgement[GameState]) = Some(AckConfig(longTimeout, ack))
    state match {
