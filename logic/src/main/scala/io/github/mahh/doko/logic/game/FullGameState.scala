...
object FullGameState {
  ...
  private[game] sealed trait AbstractFullGameState[ClientGS <: GameState, ClientPS]
    ...
    final def playerStates: TableMap[ClientGameState] = clientPlayerStates.map { s =>
      gameState(Some(s))
    }
  ...
  private[game] case class Negotiating private[game] (
    ...
    rules: Rules
      ...
      GameState,
      (Seq[Card], Either[Seq[Reservation], Option[Reservation]])
    ] {
    ...
    override protected def gameState(playerState: Option[ClientPlayerState]): GameState = {
      playerState.fold[GameState] {
        ...
        reservationState.fold(
          res =>
            GameState.AskingForReservations(
              Some(GameState.AskingForReservations.PlayerState(hand, res))
            ),
          res =>
            GameState.WaitingForReservations(
              Some(GameState.WaitingForReservations.PlayerState(hand, res))
            )
        )
      }
    }
    ...
    override def handleAction: PartialFunction[TransitionParams, FullGameState] = {
      case (pos, PlayerAction.CallReservation(r)) if players(pos).canCall(r) =>
        ...
        if (updatedPlayers.values.forall(_.reservationState.isRight)) {
          NegotiationsResult(starter, updatedPlayers, trumps, totalScores)(using rules)
        } else {
  ...
  private[game] case class NegotiationsResult(
    ...
    rules: Rules
      ...
      GameState.ReservationResult,
      GameState.ReservationResult.PlayerState
    ] {
    ...
    def apply(
      ...
      def collectReservation[R <: Reservation: ClassTag]: Option[(PlayerPosition, Reservation)] = {
        PlayerPosition
          .trickOrder(starter)
          .flatMap { p =>
            players(p).reservationState.toOption.flatten.collect { case r: R =>
              p -> r
            }
          }
          .headOption
      ...
      val updatedPlayers: TableMap[Playing.PlayerState] = players.mapWithPos { (pos, state) =>
        val hand =
          if (newTrumps == trumps) state.hand else state.hand.sorted(using newTrumps.cardsOrdering)
    ...
    override def handleAction: PartialFunction[TransitionParams, FullGameState] = {
      case (pos, PlayerAction.AcknowledgePovertyRefused) =>
        ...
        if (stillPending.isEmpty) {
          Negotiating.withDealtCards(starter, totalScores)(using rules)
        } else {
  ...
  private[game] case class PovertyExchange(
    ...
    rules: Rules
      ...
      GameState.PovertyExchange,
      GameState.PovertyExchange.PlayerState
    ] {

    private val (onOffer, poorPlayersCards) =
      players(poorPlayer).hand.partition(Trumps.Default.isTrump)
    ...
    override def handleAction: PartialFunction[TransitionParams, FullGameState] = {
      case (`acceptingPlayer`, PlayerAction.PovertyReturn) if canReturn =>
        ...
        Playing(starter, updatedPlayers, None, trumps, totalScores)(using rules)
  ...
  private[game] case class Playing(
    ...
    private val bids: Map[PlayerPosition, NameableBid] =
      players.toMap.flatMap { case (pos, state) =>
        state.bid.map(pos -> NameableBid(Role.isElders(state.role), _))
      }
    ...
    override def handleAction: PartialFunction[TransitionParams, FullGameState] = {
      case (pos, PlayerAction.PlayCard(c)) if playableCards(pos).contains(c) =>
        ...
        updatedTrick.asCompleteTrick.fold {
          ...
          copy(
            ...
            finishedTrickOpt = Some(finishedTrick)
          )
        }
      ...
      case (pos, PlayerAction.PlaceBid(b))
          if possibleBids.get(pos).exists(Bid.ordering.lteq(_, b)) =>
  ...
  private[game] object Playing {
    ...
    private implicit class RichPlayersMap(private val players: TableMap[PlayerState])
      extends AnyVal {
  ...
  private[game] case class RoundResults(
    ...
    override def handleAction: PartialFunction[TransitionParams, FullGameState] = {
      case (pos, PlayerAction.AcknowledgeRoundResult) =>
        val stillMissing = missingAcks - pos
        if (stillMissing.isEmpty) {
          Negotiating.withDealtCards(PlayerPosition.next(starter), totalScores)(using rules)
        } else {
          copy(missingAcks = stillMissing)
        }
    }
